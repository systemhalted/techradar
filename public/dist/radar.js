function init(h, w) {
    ...
    function legendPositionForQuadrant(range, index) {
        ...
        var defaultTop = 18;
        if (range.start === 0) {
            return { left: w - 200 + 30, top: defaultTop };
        }
        if (range.start === 90) {
            return { left: defaultLeft, top: defaultTop };
        }
        if (range.start === 180) {
            return { left: defaultLeft, top: h / 2 + defaultTop };
        }
        if (range.start === 270) {
            return { left: w - 200 + 30, top: h / 2 + defaultTop };
        }
    ...
    for (var i = 0; i < radar_data.length; i++) {
        ...
        for (var ringIndex = 0; ringIndex < radar_arcs.length; ringIndex++) {
            ...
            radar.add(pv.Label)
                ...
                .shape(function (d) { return (d.movement === 't' ? "triangle" : "circle"); })
                .cursor(function (d) { return (d.url !== undefined ? "pointer" : "auto"); })
                .event("click", function (d) { if (d.url !== undefined) {
                self.location = d.url;
            } })
                .size(fontSize)
                .angle(45)
